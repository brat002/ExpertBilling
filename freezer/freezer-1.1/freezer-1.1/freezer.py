...
def _getModules():
	...
	if _flags[ 'PROTECTION_MODULE' ]: mf.load_file( _flags[ 'PROTECTION_MODULE' ] )
	mf.run_script( _flags[ 'PY_FILE' ] )
	missing = mf.any_missing()
	if missing: raise ImportError( "missing modules %r" % missing )
	print( '==================================================' )
	...
	if _flags[ 'FREEZE_LOCAL' ]:
		for m in mf.modules.itervalues():
			if _flags[ 'PROTECTION_MODULE' ] and m.__file__ == _flags[ 'PROTECTION_MODULE' ]: continue
			if m.__name__ != '__main__' and m.__code__ and m.__file__.startswith( _flags[ 'SEARCH_PATH' ] ):
				result.append( m )
	print( '==================================================' )
...
import sys, marshal
sys.path[ 0 ] += '%smodules'
exec marshal.loads( %s ) in globals()""" % ( os.path.sep, repr( bytecode ) ),
				'',
				'exec'
			))
...
import sys, marshal
sys.path[ 0 ] += '%smodules'
exec marshal.loads( %s ) in globals()""" % ( # we need a way to add a local path: ugly some, but it works
			os.path.sep,
			repr( _dumpCode( _compileFile( _flags[ 'PROTECTION_MODULE' ] ) ) )
		),
		'',
		'exec'
	))
...
def _copyModules():
	def getFilesNames( mod ): return (
		mod.__name__.replace( '.', os.path.sep )
			+ ( os.path.sep + '__init__' if '__init__' in mod.__file__ else '' )
...
def _analizeFlags():
	...
	for k, v in mods.iteritems(): mods[ k ] = normalizePath( v )
