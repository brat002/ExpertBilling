...
class cdDrawer(object):
    cdchartoptdict = {"default": {'xychart':(800, 450)},\
		      ...
		      "nfs_user_speed": \
				      {'xychart':(800, 450), 'setplotarea':(100, 85, 650, 200, 0xffffff, -1, 0xc0c0c0, 0xc0c0c0, -1), 'setcolors':pychartdir.defaultPalette, \
				       'addlegend':(50, 30, 0, "LiberationMono-Regular.ttf", 14), 'legendbackground':pychartdir.Transparent,  'addtitle':("Скорость", "LiberationMono-Regular.ttf", 18), \
				       'yaxissettitle':("Скорость", "LiberationMono-Regular.ttf", 18), 'yaxissetwidth':2, 'yaxissetlabelformat':'{value|2.}',\
				       'xaxissettitle':("Время", "LiberationMono-Regular.ttf", 14), 'xaxissetwidth':2, 'xaxissetlabelformat':'',\
				       'xaxissetlabelstyle':("LiberationMono-Regular.ttf",), 'yaxissetlabelstyle': ("LiberationMono-Regular.ttf",), \
				       'autoticks': True, \
				       'outfill': True, \
		      ...
		      "nfs_port_speed": \
				      {'xychart':(800, 450), 'setplotarea':(100, 85, 650, 200, 0xffffff, -1, 0xc0c0c0, 0xc0c0c0, -1), 'setcolors':pychartdir.defaultPalette, \
				       'addlegend':(50, 30, 0, "LiberationMono-Regular.ttf", 14), 'legendbackground':pychartdir.Transparent,  'addtitle':["Скорость по %s порту:", "LiberationMono-Regular.ttf", 18], \
				       'yaxissettitle':("Скорость", "LiberationMono-Regular.ttf", 18), 'yaxissetwidth':2, 'yaxissetlabelformat':'{value|2.}',\
				       'xaxissettitle':("Время", "LiberationMono-Regular.ttf", 14), 'xaxissetwidth':2, 'xaxissetlabelformat':'',\
				       'xaxissetlabelstyle':("LiberationMono-Regular.ttf",), 'yaxissetlabelstyle': ("LiberationMono-Regular.ttf",), \
				       'autoticks': False, \
		      ...
		      "nfs_total_nass_traf":\
				       ...
				       'antialias': False, \
				       'addlinelayer_total':(0x0000FF, "TOTAL"), 'setlinewidth_total':1.3},\
		      "userstrafpie":     {'piechart':(360, 300), 'setpiesize': (180, 140, 100), 'addtitle': ('Объем трафика', "LiberationMono-Regular.ttf", 14), 'setlabelstyle':("LiberationMono-Regular.ttf",)}, \
		      "sessions":         {'xychart':(640, 280), 'addtitle':("Сессии", "LiberationMono-Regular.ttf", 18), 'setplotarea':(160, 55, 460, 200, 0xffffff, -1, -1, 0xc0c0c0, 0xc0c0c0), \
					   'xaxissetlabelstyle':("LiberationMono-Regular.ttf",), 'yaxissetlabelstyle': ("LiberationMono-Regular.ttf",), \
		      ...
		      "trans_deb":        {'xychart':(620, 310), 'addtitle':("Транзакции: дебет", "LiberationMono-Regular.ttf", 18), 'setplotarea':(140, 55, 460, 200, 0xffffff, -1, -1, 0xc0c0c0, 0xc0c0c0), \
					   'setcolors':pychartdir.defaultPalette, 'addlegend':(150, 20, 0, "LiberationMono-Regular.ttf", 14), 'legendbackground':pychartdir.Transparent,\
					   'xaxissetlabelstyle':("LiberationMono-Regular.ttf",), 'yaxissetlabelstyle': ("LiberationMono-Regular.ttf",), \
					   'autoticks': True, \
					   'antialias': True, \
		      ...
		      "trans_crd":        {'xychart':(620, 310), 'addtitle':("Транзакции: кредит", "LiberationMono-Regular.ttf", 18), 'setplotarea':(140, 55, 460, 200, 0xffffff, -1, -1, 0xc0c0c0, 0xc0c0c0), \
					   'setcolors':pychartdir.defaultPalette, 'addlegend':(150, 20, 0, "LiberationMono-Regular.ttf", 14), 'legendbackground':pychartdir.Transparent,\
					   'xaxissetlabelstyle':("LiberationMono-Regular.ttf",), 'yaxissetlabelstyle': ("LiberationMono-Regular.ttf",), \
					   'autoticks': True, \
					   'antialias': True, \
	...
	try:
	    #get a string from #selstrdict# dictionary wit a key based on the method name and compute a query string from it 
	    selstr = selstrdict['nfs'] % (', direction', '',  args[0].isoformat(' '), args[1].isoformat(' '), (((len(args)-2)==1) and ("AND nas_id IN (%s)" % ', '.join([str(int) for int in args[2]]))) or  (((len(args)-2)==0) and ' '))
	except Exception, ex:
	    raise ex
        data = bpbl.get_traf(selstr, kwargs.has_key('sec') and kwargs['sec'])
	if not data: print "Dataset is empty"; return []
	...
	try:
	    #get a string from #selstrdict# dictionary wit a key based on the method name and compute a query string from it 
	    selstr = selstrdict['nfs'] % (', account_id', "(account_id IN (%s)) AND" % ', '.join([str(int) for int in args[0]]), args[1].isoformat(' '), args[2].isoformat(' '), (((len(args)-3)==1) and ("AND (nas_id IN (%s))" % ', '.join([str(int) for int in args[3]]))) or  (((len(args)-3)==0) and ' '))
	except Exception, ex:
	    raise ex
	...
	try:
	    #get a string from #selstrdict# dictionary wit a key based on the method name and compute a query string from it 
	    selstr = selstrdict['nfs'] % ('', '', args[0].isoformat(' '), args[1].isoformat(' '), (((len(args)-2)==1) and ("AND nas_id IN (%s)" % ', '.join([str(int) for int in args[2]]))) or  (((len(args)-2)==0) and ' '))
	except Exception, ex:
	    raise ex
	...
	try:
	    pts = ', '.join(str(pint) for pint in args[0])
	    selstr = selstrdict['nfs_port_speed'] % (pts, pts, args[1].isoformat(' '), args[2].isoformat(' '))
	except Exception, ex:
	    raise ex
	...
	try:
	    ...
	    #get a string from #selstrdict# dictionary with a key based on the method name and compute a query string from it 
	    selstr = selstrdict['nfs'] % (', direction', '', args[1].isoformat(' '), args[2].isoformat(' '), "AND (nas_id=%d)" % args[0])
	except Exception, ex:
	    raise ex
	data = bpbl.get_traf(selstr, kwargs.has_key('sec') and kwargs['sec'])	
	if not data: print "Dataset is empty"; return []
	...
	try:
	    #get a string from #selstrdict# dictionary wit a key based on the method name and compute a query string from it 
	    selstr = selstrdict['nfs'] % (', nas_id', '', args[1].isoformat(' '), args[2].isoformat(' '), ("AND (nas_id IN (%s))" % ', '.join([str(int) for int in args[0]])))
	except Exception, ex:
	    raise ex
        data = bpbl.get_total_users_traf(selstr, kwargs.has_key('sec') and kwargs['sec'])
	print "----------------------------"
	...
	#Add in line layer 
	#for key in y_total_n.iterkeys():
	    #layer = c.addLineLayer(y_total_n[key], -1, key)
	    #layer.setXData(times)        
	    ## Set the line width	
	    #layer.setLineWidth(optdict['setlinewidth_total'])    
	...
	try:
	    selstr = selstrdict['userstrafpie'] % (args[1].isoformat(' '), args[2].isoformat(' '), ', '.join([str(int) for int in args[0]]))	
	except Exception, ex:
	    raise ex
	...
	try:
	    selstr = selstrdict['sessions'] % (', '.join([str(int) for int in args[0]]), args[1].isoformat(' '), args[2].isoformat(' '), args[1].isoformat(' '), args[2].isoformat(' '))
	except Exception, ex:
	    raise ex
	...
	startDate = chartTime(args[1].year, args[1].month, args[1].day, args[1].hour, args[1].minute, args[1].second)
	endDate   = chartTime(args[2].year, args[2].month, args[2].day, args[2].hour, args[2].minute, args[2].second)
