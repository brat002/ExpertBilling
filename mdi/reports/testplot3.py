...
class MainWindow(QtGui.QMainWindow):
    ...
    def createLetter(self, name, address, orderItems, sendOffers):
        ...
        testperf = time.clock()
        surface = cairo.ImageSurface(cairo.FORMAT_ARGB32, 640, 300)
        a1 = [1,2,3,4]
        a2 = [3,5,2,8]
        dr = Drawer()
        ...
        yin = [(times[i], y_in[i]) for i in range(len(times))]
        yout = [(times[i], y_out[i]) for i in range(len(times))]
        ds1 = (
            ('yin', yin),
            )
        #ds2 = (            ('yout', yout),            )
