...
class FinanceChart(MultiChart) :
    ...
    def addVolBars2(self, c, height, upColor, downColor, flatColor) :
        ...
        upLayer = self.addBarIndicator2(c, ArrayMath(self.m_volData).selectGTZ(closeChange).result(
            ), upColor, "")
        dnLayer = self.addBarIndicator2(c, ArrayMath(self.m_volData).selectLTZ(closeChange).result(
            ), downColor, "")
        flatLayer = self.addBarIndicator2(c, ArrayMath(self.m_volData).selectEQZ(closeChange
            ).result(), flatColor, "")

        maxVol = ArrayMath(self.m_volData).max()
        ...
        volFormat = ""
        if maxVol < 10 :
            volFormat = "{value|1}%s" % (self.m_volUnit)
        else :
            volFormat = "{value}%s" % (self.m_volUnit)
        a.setLabelFormat(volFormat)

        i = self.lastIndex(self.m_volData)
        label = "Vol"
        if i >= 0 :
            label = "Vol: %s%s" % (self.formatValue(self.m_volData[i], self.m_generalFormat),
                self.m_volUnit)
    ...
    def addIndicator(self, height) :
        ...
        if self.m_currentChart != None :
            ...
            self.m_currentChart.xAxis().setColors(LineColor, Transparent)
        ...
        ret.setPlotArea(self.m_leftMargin, self.m_topMargin,
            ...
            self.m_plotAreaBgColor).setGridColor(self.m_majorHGridColor, self.m_majorVGridColor,
    ...
    def addChaikinOscillator(self, height, color) :
        ...
        expAvg10 = ArrayMath(accdist).expAvg(2.0 / (11)).result()
        return self.addLineIndicator(height, ArrayMath(accdist).expAvg(2.0 / (4)).sub(expAvg10
    ...
    def addPerformance(self, height, color) :
        firstCloseIndex = self.m_extraPoints
        while firstCloseIndex < len(self.m_closeData) :
            if (self.m_closeData[firstCloseIndex] != NoValue) and (self.m_closeData[firstCloseIndex
                ] != 0) :
                break
            firstCloseIndex = firstCloseIndex + 1

        if firstCloseIndex < len(self.m_closeData) :
            return self.addLineIndicator(height, ArrayMath(self.m_closeData).div(self.m_closeData[
                firstCloseIndex]).sub(1).mul(100).result(), color, "Performance")
        else :
            #chart is empty !!!
    ...
    def computeRSI(self, period) :
        ...
        absChange = ArrayMath(self.m_closeData).delta().abs().movAvg(period).result()
        return ArrayMath(self.m_closeData).delta().selectGTZ().movAvg(period).financeDiv(absChange,
            0.5).mul(100).result()
