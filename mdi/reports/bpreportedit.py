...
class reportConstructor(Dispatcher, ContentHandler):
    ...
    def startStable(self, attrs):
        ...
        if attrs['ftype'] == 'new':
            ...
            tformat = self.objdict['tableformat'][attrs['fname']]
        else:
            raise Exception("startStructureTable: no such ftype " + attrs['ftype'])
        table = self.cursor.insertTable(int(attrs['rownum']), int(attrs['colnum']), tformat)
        table.cframepos = cframepos
        table.tpos = self.cursor.position()
        self.structTables.append(table)
    def endStable(self):
        table = self.structTables.pop()
        self.cursor.setPosition(table.cframepos)
    def startScell(self, attrs):
        table = self.structTables[-1]
        self.cursor.setPosition(table.tpos)
        self.cursor = table.cellAt(int(attrs['row']), int(attrs['col'])).firstCursorPosition()
    def endScell(self):
        table = self.structTables[-1]
        self.cursor.setPosition(table.tpos)
    ...
    def endTable(self):	
        ...
        if self.objdict['curtable']['type'] == 'normal':
            ...
            for tbl in range(len(self.curdata)):
                ...
                self.cursor.setPosition(self.cursor.currentFrame().parentFrame().lastPosition())		
        #move cursor to the utmost position in the frame
