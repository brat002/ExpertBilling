...
class CorePacket(UserDict.UserDict):
	...
	def __init__(self, code=0, secret='',username="", password="", **attributes):
		"""Constructor
	...
	def ReplyPacket(self):
		...
		#attr=attr+attrs
		#headerlength|code|ulength|username|plength|password|secretl|secret|attrlength
		userlength=len(self.username)
		passwordlength=len(self.password)
	...
	def DecodePacket(self, packet):
		...
		try:
		    ...
		    secondpart=packet[6:6+ulength+1]
		    
		    (self.username,plength)=struct.unpack("!%dsB" % ulength, secondpart)
		    
		    thirdpart=packet[len(secondpart)+len(firstpart):len(firstpart)+len(secondpart)+1+plength]
		    
		    (self.password,secretl)=struct.unpack("!%dsB" % plength,thirdpart)
		    
		    fourthpart=packet[len(firstpart)+len(secondpart)+1+plength:headerlength]
		    
		    (self.secret,attrlen)=struct.unpack("!%dsH" % secretl, fourthpart)
		    
		except struct.error:
			raise PacketError, "Packet header is corrupt"
		if len(packet)!=attrlen+headerlength:
