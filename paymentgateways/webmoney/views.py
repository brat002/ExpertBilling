...
def simple_payment(request):
    ...
    if request.method == 'POST':
        ...
        initial = {
            ...
            'LMI_PAYMENT_NO': Payment.objects.create(account=request.user.account, amount=amount).id,
...
def result(request):
    ...
    if form.is_valid():
        ...
        if generated_hash == form.cleaned_data['LMI_HASH']:
            ...
            payment.telepat_phonenumber=form.cleaned_data['LMI_TELEPAT_PHONENUMBER']
            ...
            try:
                ...
                mail_admins('Unprocessed payment without invoice!',
                            'Payment NO is %s.\npayment:\n%s\ndata:\n%s' % payment_no,repr(payment),repr(form.cleaned_data),
                            fail_silently=True)
