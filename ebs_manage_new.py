...
def unpack(tarfile):
    local('cp %s /opt/ebs/deploy/ebs.tar' % tarfile)
    local('mkdir -p /opt/ebs/deploy/billing')
    local('rm -rf /opt/ebs/deploy/billing/*')
    with lcd('/opt/ebs/deploy'):
        local('tar -xvzf ebs.tar -C billing/')
    
def update_src():
    local('cp -rf /opt/ebs/deploy/billing/* /opt/ebs/data/')
...
def db_upgrade():
    ...
    SQL_UPGRADE_PATH = os.path.join(DEPLOYMENT_DIR, 'billing/sql/upgrade')
