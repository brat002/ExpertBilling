...
def contracttemplate_edit(request):
    ...
    if request.method == 'POST': 
        ...
        if  not (request.user.account.has_perm('billservice.add_tariff')):
                messages.error(request, _(u'У вас нет прав на создание тарифного плана'), extra_tags='alert-danger')
                return HttpResponseRedirect(request.path)
        ...
        if form.is_valid():
            ...
            messages.success(request, _(u'Шаблон номера договора успешно создан.'), extra_tags='alert-success')
            return HttpResponseRedirect(reverse("contracttemplate"))
        else:
            messages.error(request, _(u'При сохранении шаблона номера договора возникли ошибки.'), extra_tags='alert-danger')
            return {'form':form,  'status': False, 'item': model} 

    else:
        id = request.GET.get("id")
        if  not (request.user.account.has_perm('billservice.view_tariff')):
            messages.error(request, _(u'У вас нет прав на доступ на просмотр тарифных планов.'), extra_tags='alert-danger')
            return HttpResponseRedirect('/ebsadmin/')
        if id:
...
def contracttemplate_delete(request):
    ...
    if id:
        try:
            ...
            return {"status": False, "message": _(u"Указанный шаблона номера договора не найден %s") % str(e)}
        log('DELETE', request.user, item)
        item.delete()
        messages.success(request, _(u'Шаблон номера договора успешно удалён.'), extra_tags='alert-success')
        return {"status": True}
    else:
        messages.error(request, _(u'При удалении шаблона номера договора возникли ошибки.'), extra_tags='alert-danger')
