...
from payments.qiwiru.forms import (
    AdditionalFieldsForm,
    CheckAdditionalFieldsForm
)
...
params = {
    'create_invoice': CREATE_INVOICE_TEMPLATE,
    'get_invoices_status': GET_INVOICE_STATUS_TEMPLATE,
    'get_invoices': GET_INVOICES_TEMPLATE,
    'response': RESPONSE_TEPMLATE,
    'response_error': RESPONSE_ERROR_TEPMLATE
}
...
class PaymentProcessor(PaymentProcessorBase):
    ...
    def get_gateway_url(self, request, payment):
        ...
        _, _ = PaymentProcessor.create_invoice(
    ...
    def postback(request):
        ...
        if data['WMI_ORDER_STATE'] == 'Accepted' and \
            ...
            return (u'WMI_RESULT=RETRY&WMI_DESCRIPTION=' +
                    _(u'Ошибка обработки платежа'))

    @staticmethod
    def check(request):
        if not request.GET:
            return PaymentProcessor.error(None, 300)
