...
class PaymentProcessor(PaymentProcessorBase):
    ...
    def online(ip, id, tr_id, tr_date, tr_crc, tr_amount, tr_paid, tr_desc,
        ...
        if md5sum != PaymentProcessor.compute_sig(
                params, PaymentProcessor._ONLINE_SIG_FIELDS, key):
        ...
        try:
            ...
            logger.error(('Got message with CRC set to non '
                          'existing Payment, %s') % str(params))
            return 'CRC ERR'

        logger.info(('Incoming payment: id=%s, tr_id=%s, tr_date=%s, '
                     'tr_crc=%s, tr_amount=%s, tr_paid=%s, tr_desc=%s, '
                     'tr_status=%s, tr_error=%s, tr_email=%s') %
        ...
        if tr_status == 'TRUE':
            # Due to Transferuj documentation, we need to check if amount is
            # correct
    ...
    def get_gateway_url(self, request):
        ...
        params = {
            ...
            'opis': self.get_order_description(
        ...
        if signing:
            params['md5sum'] = PaymentProcessor.compute_sig(
                params, self._REQUEST_SIG_FIELDS, key)

        current_site = Site.objects.get_current()

        if PaymentProcessor.get_backend_setting('force_ssl_online', False):
            params['wyn_url'] = 'https://' + current_site.domain + \
                reverse('getpaid-transferuj-online')
        else:
            params['wyn_url'] = 'http://' + current_site.domain + \
                reverse('getpaid-transferuj-online')

        if PaymentProcessor.get_backend_setting('force_ssl_return', False):
            params['pow_url'] = 'https://' + current_site.domain + \
                reverse('getpaid-transferuj-success',
                        kwargs={'pk': self.payment.pk})
            params['pow_url_blad'] = 'https://' + current_site.domain + \
                reverse('getpaid-transferuj-failure',
                        kwargs={'pk': self.payment.pk})
        else:
            params['pow_url'] = 'http://' + current_site.domain + \
                reverse('getpaid-transferuj-success',
                        kwargs={'pk': self.payment.pk})
            params['pow_url_blad'] = 'http://' + current_site.domain + \
                reverse('getpaid-transferuj-failure',
                        kwargs={'pk': self.payment.pk})
