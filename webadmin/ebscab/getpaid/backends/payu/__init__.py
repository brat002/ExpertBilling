...
class PaymentProcessor(PaymentProcessorBase):
    ...
    _REQUEST_SIG_FIELDS = ('pos_id', 'pay_type', 'session_id', 'pos_auth_key',
                           'amount', 'desc', 'desc2', 'trsDesc', 'order_id',
                           'first_name', 'last_name', 'payback_login',
                           'street', 'street_hn', 'street_an', 'city',
                           'post_code', 'country', 'email', 'phone',
                           'language', 'client_ip', 'ts')
    ...
    def online(pos_id, session_id, ts, sig):
        ...
        try:
            ...
            return 'POS_ID ERR'
        if params['pos_id'] != int(PaymentProcessor
                                   .get_backend_setting('pos_id')):
            return 'POS_ID ERR'
    ...
    def get_payment_status(self, session_id):
        ...
        for tag in tag_response.childNodes:
            if tag.nodeType == Node.ELEMENT_NODE:
                response_params[tag.nodeName] = reduce(
                    lambda x, y: x + y.nodeValue, tag.childNodes, u"")
        if PaymentProcessor.compute_sig(
                response_params, self._GET_RESPONSE_SIG_FIELDS, key2) == \
                response_params['sig']:
            if not (int(response_params['pos_id']) == params['pos_id'] or
                    int(response_params['order_id']) == self.payment.pk):
                logger.error(('Wrong pos_id and/or payment for Payment/get '
                              'response data %s') % str(response_params))
                return
            ...
            if status == PayUTransactionStatus.FINISHED:
                self.payment.amount_paid = Decimal(
                    response_params['amount']) / Decimal('100')
                self.payment.paid_on = \
                    datetime.datetime.utcnow().replace(tzinfo=utc)
                if Decimal(response_params['amount']) / Decimal('100') >= \
                        self.payment.amount:
            ...
            logger.error(('Wrong signature for Payment/get '
                          'response data %s') % str(response_params))
