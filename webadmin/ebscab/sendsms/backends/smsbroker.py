...
class SmsBackend(BaseSmsBackend):
    ...
    def __init__(self, fail_silently=False, **kwargs):
        ...
        self.service_number = get_backend_settings(self.name).get('SERVICE_NUMBER')
        self.operator = get_backend_settings(self.name).get('OPERATOR')
        self.validity_period = get_backend_settings(self.name).get('VALIDITY_PERIOD')
    ...
    def send_message(self, message):
        ...
        <bulk-request login="%(login)s" password="%(password)s" ref-id="%(ref-id)s" delivery-notification-requested="true" version="1.0">
            <message id="1"
                     ...
                     priority="%(priority)s">
<content type="text/plain">%(content)s</content> </message></bulk-request>
        """
