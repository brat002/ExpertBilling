...
from paymentgateways.qiwi.xml_helper import xml2obj
...
result_codes = {
    ...
    '-2': _(u'Произошла ошибка. Счёт не может быть подтверждён. Возможно у вас '
            u'недостаточно средств или включено подтверждение действий по SMS'),
    '0': _(u'Успех'),
    ...
    '353': _(u'Включено SMS подтверждение действий. Невозможно проверить '
             u'баланс.'),
...
def create_invoice(phone_number, transaction_id, summ=0, comment='',
                   lifetime=48):
    xml = make_request(params['create_invoice'] % (phone_number,
                                                   summ,
                                                   transaction_id,
                                                   lifetime,
                                                   comment))
