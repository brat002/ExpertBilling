...
ERRCODES = {
    "0": _(u"Успешное выполнение операции"),
    "1": _(u"Платеж уже был проведен"),
    "10": _(u"Запрос выполнен с неразрешенного адреса"),
    "11": _(u"Указаны не все необходимые параметры "),
    "12": _(u"Неверный формат параметров"),
    "13": _(u"Неверная цифровая подпись"),
    "20": _(u"Указанный номер счета отсутствует"),
    "21": _(u"Запрещены платежи на указанный номер счета"),
    "22": _(u"Запрещены платежи для указанной услуги"),
    "23": _(u"Запрещены платежи для указанного агента"),
    "29": _(u"Неверные параметры платежа. В тэге err_text конкретное "
            u"описание причины"),
    "30": _(u"Был другой платеж с указанным номером"),
    "90": _(u"Временная техническая ошибка"),
    "99": _(u"Прочие ошибки Оператора. В тэге err_text указывается "
            u"описание причины"),
...
def payment(request):
    ...
    if not sign:
        return HttpResponse(BODY_ERR % CHECK_BODY_ERROR % ({
            "ERR_CODE": 11,
            "ERR_TEXT": ERRCODES.get("11")
        }))
    res = checkSign(xml)
    if not res:
        return HttpResponse(BODY_ERR % CHECK_BODY_ERROR % ({
            "ERR_CODE": 13,
            "ERR_TEXT": ERRCODES.get("13")
        }))
    ...
    if amount <= 0:
        response = CHECK_BODY_ERROR % ({
            "ERR_CODE": 12,
            "ERR_TEXT": _(u"Сумма должна быть больше нуля")
        })
        return HttpResponse(BODY % (response, genSign(response)))
    ...
    try:
        ...
        response = CHECK_BODY_ERROR % ({
            "ERR_CODE": 20,
            "ERR_TEXT": ERRCODES.get("20")
        })
        return HttpResponse(BODY % (response, genSign(response)))

    if action == CHECK:
        response = CHECK_BODY_SUCCESS % ({
            "ERR_CODE": 0,
    ...
    if action == PAYMENT:
        ...
        try:
            ...
            response = CHECK_BODY_ERROR % ({
                "ERR_CODE": 11,
                "ERR_TEXT": _(u"Неверный формат даты %s" % e)
            })
        ...
        try:
            ...
            response = CHECK_BODY_ERROR % ({
                "ERR_CODE": 99,
                "ERR_TEXT": _(u"Ошибка проведения платежа %s" % e)
            })
