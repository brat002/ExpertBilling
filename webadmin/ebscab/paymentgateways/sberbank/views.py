...
ERRCODES = {
    ...
    "29": (u"Неверные параметры платежа. В тэге err_text конкретное "
           u"описание причины"),
    "30": u"Был другой платеж с указанным номером",
    "90": u"Временная техническая ошибка",
    "99": (u"Прочие ошибки Оператора. В тэге err_text указывается "
           u"описание причины"),
...
def payment(request):
    ...
    if amount <= 0:
        response = CHECK_BODY_ERROR % ({
            "ERR_CODE": 12,
            "ERR_TEXT": u"Сумма должна быть больше нуля"
        })
        return HttpResponse(BODY % (response, genSign(response)))
    ...
    if action == PAYMENT:
        ...
        try:
            ...
            response = CHECK_BODY_ERROR % ({
                "ERR_CODE": 99,
                "ERR_TEXT": u"Ошибка проведения платежа %s" % e
            })
