    ...
    if request.method == 'POST':
        if id:
            ...
            if not (request.user.account.has_perm(
                    'billservice.change_hardwaretype')):
                messages.error(
                    request,
                    _(u'У вас нет прав на редактирование типов оборудования'),
                    extra_tags='alert-danger')
                return HttpResponseRedirect(request.path)
        else:
            form = HardwareTypeForm(request.POST)
            if not (request.user.account.has_perm(
                    'billservice.add_hardwaretype')):
                messages.error(
                    request,
                    _(u'У вас нет прав на создание типов оборудования'),
                    extra_tags='alert-danger')
                return HttpResponseRedirect(request.path)
...
def hardwaretype_delete(request):
    if not (request.user.account.has_perm('billservice.delete_hardwaretype')):
        return {
            'status': False,
            'message': _(u'У вас нет прав на удаление типов оборудования пулов')
        }
    ...
    if id:
        try:
            ...
            return {
                "status": False,
                "message": _(u"Указанный тип оборудования не найден %s") % str(e)
            }
        ...
        return {
            "status": False,
            "message": _(u'HardwareType not found')
        }
