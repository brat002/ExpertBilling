...
from ebsadmin.tables import SettlementPeriodTable
...
def settlementperiod(request):
    if not (request.user.account.has_perm(
            'billservice.view_settlementperiod')):
        messages.error(
            request,
...
def settlementperiod_edit(request):
    ...
    if request.method == 'POST':
        ...
        if id:
            if not (request.user.account.has_perm(
                    'billservice.change_settlementperiod')):
                messages.error(
                    request,
                    _(u'У вас нет прав на редактирование расчётных периодов'),
                    extra_tags='alert-danger')
                return HttpResponseRedirect(request.path)

        else:
            if not (request.user.account.has_perm(
                    'billservice.add_settlementperiod')):
                messages.error(
                    request,
                    _(u'У вас нет прав на создание расчётных периодов'),
                    extra_tags='alert-danger')
                return HttpResponseRedirect(request.path)
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                _(u'Расчётный период удачно создан.'),
        ...
        if id:
            ...
            form = SettlementPeriodForm(
                initial={'time_start': datetime.datetime(2010, 1, 1, 0, 0, 0)})
...
def settlementperiod_delete(request):
    if not (request.user.account.has_perm(
            'billservice.delete_settlementperiod')):
        return {
            'status': False,
            'message': _(u'У вас нет прав на удаление расчётных периодов')
        }
    ...
    if id:
        try:
            ...
            return {
                "status": False,
                "message": _(u"Указанный расчётный период не найден %s") % str(e)
            }
