...
def timeperiod_edit(request):
    ...
    if id:
        if not (request.user.account.has_perm(
                'billservice.change_timeperiod')):
            return {
                'status': False,
                'message': _(u'У вас нет прав на изменение периодов')
            }
        ...
        if not (request.user.account.has_perm('billservice.add_timeperiod')):
            return {
                'status': False,
                'message': _(u'У вас нет прав на добавление периодов')
            }
...
def timeperiodnode_edit(request):
    ...
    if prefix == 'TP':
        return {
            "status": True,
            "form": render_to_string('ebsadmin/timeperiodnode_edit.html', {
                'form': TimePeriodNodeForm(initial={'time_period': id})
            })
        }
    ...
    if request.method == 'POST':
        ...
        if id:
            if not (request.user.account.has_perm(
                    'billservice.change_timeperiodnode')):
                return {
                    'status': False,
                    'message': _(u'У вас нет прав на редактирование подпериодов')
                }
        else:
            if not (request.user.account.has_perm(
                    'billservice.add_timeperiodnode')):
                return {
                    'status': False,
                    'message': _(u'У вас нет прав на добавление подпериодов')
                }
        ...
        if not (request.user.account.has_perm(
                'billservice.view_timeperiodnode')):
            return {
                'status': True
            }
        key = request.GET.get('key', '')
            ...
            form = TimePeriodNodeForm(
                initial={
                    "time_end": (item.time_start +
                                 datetime.timedelta(seconds=item.length))
                },
                instance=item)
        else:
            form = TimePeriodNodeForm()
...
def timeperiod_delete(request):
    ...
    if prefix == 'TP':
        if not (request.user.account.has_perm(
                'billservice.delete_timeperiod')):
            return {
                'status': False,
                'message': _(u'У вас нет прав на удаление периодов')
            }
    ...
    if prefix == 'NODE':
        if not (request.user.account.has_perm(
                'billservice.delete_timeperiodnode')):
            return {
                'status': False,
                'message': _(u'У вас нет прав на удаление подпериодов')
            }
