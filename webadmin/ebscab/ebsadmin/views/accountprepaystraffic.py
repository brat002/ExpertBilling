...
def accountprepaystraffic(request):
    ...
    if request.method == 'GET' and request.GET:
        ...
        if form.is_valid():
            ...
            res = (AccountPrepaysTrafic.objects
                   .all()
    ...
    if request.method == 'POST':
        if id:
            ...
            if not (request.user.account.has_perm(
                    'billservice.change_accountprepaystraffic')):
                messages.error(request,
            ...
            if not (request.user.account.has_perm(
                    'billservice.add_accountprepaystraffic')):
                messages.error(request,
                               _(u'У вас нет прав на добавление '
                                 u'предоплаченного трафика'),
                               extra_tags='alert-danger')
                return HttpResponseRedirect(request.path)
...
def accountprepaystraffic_delete(request):
    if not (request.user.account.has_perm(
            'billservice.delete_accountprepaystrafic')):
        return {
            'status': False,
            'message': _(u'У вас нет прав на удаление предоплаченного '
                         u'NetFlow трафика')
        }
