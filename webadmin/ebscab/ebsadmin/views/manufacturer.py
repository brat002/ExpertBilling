    ...
    if request.method == 'POST':
        if id:
            ...
            if not (request.user.account.has_perm(
                    'billservice.change_manufacturer')):
                messages.error(request,
                               _(u'У вас нет прав на редактирование '
                                 u'производителей оборудования'),
                               extra_tags='alert-danger')
                return HttpResponseRedirect(request.path)
        else:
            form = ManufacturerForm(request.POST)
            if not (request.user.account.has_perm(
                    'billservice.add_manufacturer')):
                messages.error(request,
                               _(u'У вас нет прав на создание '
                                 u'производителей оборудования'),
                               extra_tags='alert-danger')
                return HttpResponseRedirect(request.path)
        ...
        if form.is_valid():
            ...
            messages.success(request,
                             _(u'Производитель успешно сохранён.'),
            ...
            messages.error(request,
                           _(u'При сохранении производителя возникли ошибки.'),
                           extra_tags='alert-danger')
            return {
                'form': form,
                'status': False
        ...
        if not (request.user.account.has_perm(
                'billservice.view_manufacturers')):
            messages.error(request,
                           _(u'У вас нет прав на доступ в этот раздел.'),
                           extra_tags='alert-danger')
            return {}
...
def manufacturer_delete(request):
    if not (request.user.account.has_perm('billservice.delete_manufacturer')):
        return {
            'status': False,
            'message': _(u'У вас нет прав на удаление производителей '
                         u'оборудования')
        }
    ...
    if id:
        try:
            ...
            return {
                "status": False,
                "message": _(u"Указанный производитель оборудования "
                             u"найден %s") % str(e)
            }
        log('DELETE', request.user, item)
        item.delete()
        messages.success(request,
                         _(u'Производитель успешно удалён.'),
                         extra_tags='alert-success')
        return {
            "status": True
        }
        ...
        messages.error(request,
                       _(u'При удалении производителя возникли ошибки.'),
                       extra_tags='alert-danger')
        return {
            "status": False,
