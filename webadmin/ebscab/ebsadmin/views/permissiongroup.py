    ...
    if request.method == 'POST':
        if id:
            ...
            if not (request.user.account.has_perm(
                    'billservice.change_permissionrule')):
                messages.error(
                    request,
                    _(u'У вас нет прав на редактирование групп доступа'),
                    extra_tags='alert-danger')
                return HttpResponseRedirect(request.path)
        else:
            if not (request.user.account.has_perm(
                    'billservice.add_permissiongroup')):
                messages.error(request,
                               _(u'У вас нет прав на создание групп доступа'),
                               extra_tags='alert-danger')
...
def permissiongroup_delete(request):
    if not (request.user.account.has_perm(
            'billservice.delete_permissiongroup')):
        return {
            'status': False,
    ...
    if id:
        try:
            ...
            return {
                "status": False,
                "message": _(u"Указанная группа доступа не найдена %s") % str(e)
            }
