...
def activate_card(login, pin):
    ...
    if login and pin:
        ...
        transaction.type = (TransactionType.objects
                            .get(internal_name='ACCESS_CARD'))
    ...
    if pin and (settings.HOTSPOT_ONLY_PIN or card_id) and account_id:
        ...
        if settings.HOTSPOT_ONLY_PIN:
            ...
            card = Card.objects.filter(
                type=0, id=card_id, pin=pin, disabled=False)
        if card:
            card = card[0]
...
def add_addonservice(account_id, service_id, subaccount_id=None, ignore_locks=False, activation_date=None):
    ...
    account = account[0]
    now = datetime.datetime.now()
    accountaddonservices = (
        AccountAddonService.objects
        .filter(account=account, service__id=service_id)
        .filter(Q(deactivated__gt=now) | Q(deactivated__isnull=True)))
    ...
    if service.change_speed and ignore_locks == False:
        ...
        aas = (AccountAddonService.objects
               .filter(account=account,
                       service__change_speed=True,
    ...
    if not ignore_locks:
        if service.allow_activation == False and \
                (account.ballance <= 0 or
                 account.balance_blocked == True or
                 account.disabled_by_limit == True or
                 account.status != 1):
            return "ACCOUNT_BLOCKED"

    tarif_service = AddonServiceTarif.objects.filter(
        service=service, tarif=account.get_account_tariff())
    ...
    if tarif_service.activation_count != 0 and ignore_locks == False:
        ...
        if tarif_service.activation_count_period:
            ...
            if settlement_period.autostart:
                settlement_period.time_start = \
                    account.get_accounttariff().datetime

            settlement_period_start, settlement_period_end, delta = \
                settlement_period_info(settlement_period.time_start,
                                       settlement_period.length_in,
                                       settlement_period.length)

            aas = (AccountAddonService.objects
                   .filter(account=account,
                           service=service,
                           activated__gte=settlement_period_start,
                           activated__lte=settlement_period_end)
                   .count())
        else:
            aas = (AccountAddonService.objects
        ...
        if aas >= tarif_service.activation_count:
            return "TOO_MUCH_ACTIVATIONS"
...
def del_addonservice(account_id, account_service_id):
    ...
    if service.cancel_subscription:
        ...
        now = datetime.datetime.now()
        if (((now - accountservice.activated).seconds +
             (now - accountservice.activated).days * 86400) < delta) or \
                (service.wyte_cost and delta == 0):

            ast = AddonServiceTransaction()
            ast.account = account
            ast.type = TransactionType.objects.get(
                internal_name='ADDONSERVICE_WYTE_PAY')
