...
from billservice.models import Group, Tariff
...
chartdata = {
    'sessionsonline': {
        'name': _(u'Сессии пользователей'),
        'yname': _(u'Аккаунты'),
        'tabs': ['accountsTab', 'nassesTab']
    },
    'selectedaccountstraffic': {
        'name': _(u'Потребление трафика выбранными аккаунтами'),
        'yname': _(u'МБ'),
        'tabs': ['accountsTab', 'groupsTab'],
        'type': 'line'
    },
    'accountstraffic': {
        'name': _(u'Потребление трафика аккаунтами(общее)'),
        'yname': _(u'МБ'),
        'tabs': ['accountsTab', 'groupsTab']
    },
    'nassestraffic': {
        'name': _(u'Потребление трафика по серверам доступа'),
        'yname': _(u'МБ'),
        'tabs': ['nassesTab'],
        'type': 'line'
    },
    'tariffstraffic': {
        'name': _(u'Распределение трафика по тарифам'),
        'tabs': ['groupsTab'],
        'type': 'pie'
    },
    'distrtrafficgroups': {
        'name': _(u'Распределение трафика по группам трафика'),
        'tabs': ['accountsTab', 'groupsTab', 'nassesTab'],
        'type': 'pie'
    },
    'distraccountstraffic': {
        'name': _(u'Распределение трафика по аккаунтам '),
        'tabs': ['accountsTab', 'groupsTab'],
        'type': 'pie'
    },
    'distnassestraffic': {
        'name': _(u'Распределение трафика по серверам доступа'),
        'tabs': ['nassesTab'],
        'type': 'pie'
    },
    'distraccountstoptraffic': {
        'name': _(u'ТОП 10 по потреблению трафика '),
        'tabs': ['groupsTab'],
        'type': 'pie'
    },
    'accountsincrease': {
        'name': _(u'Прирост абонентской базы '),
        'yname': _(u'Количество'),
        'units': u'Аккаунтов',
        'tabs': [],
        'type': 'spline'
    },
    'disttransactiontypessumm': {
        'name': _(u'Распределение платежей/списаний по типам (сумма) '),
        'units': settings.CURRENCY,
        'tabs': [],
        'type': 'pie'
    },
    'disttransactiontypescount': {
        'name': _(u'Распределение платежей/списаний по типам(кол-во)'),
        'units': _(u'шт.'),
        'tabs': [],
        'type': 'pie'
    },
...
class ReportForm(forms.Form):
    report = forms.ChoiceField(
        choices=sorted([(x, chartdata[x].get('name')) for x in chartdata],
                       key=lambda k: k[1]),
        required=True,
    ...
    classes = forms.ModelMultipleChoiceField(
        queryset=TrafficClass.objects.all(), required=False)
    nasses = forms.ModelMultipleChoiceField(
        queryset=Nas.objects.all(), required=False)
    groups = forms.ModelMultipleChoiceField(
        queryset=Group.objects.all(), required=False)
    tariffs = forms.ModelMultipleChoiceField(
...
class TrafficNodesUploadForm(forms.Form):
    ...
    nodes_file = forms.FileField(
        label=_(u"Файл с сетями назначения"),
        help_text=_(u'Пример:<br/> 192.168.11.0/24<br/>172.168.0.0/16<br/>'
                    u'127.0.0.0/8'))
...
class CommentDoneForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'done_comment': forms.widgets.Textarea(attrs={
                'rows': 5,
                'class': 'input-large span5'
            }),
            'due_date': forms.widgets.DateTimeInput(attrs={
