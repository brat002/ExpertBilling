...
class TimePeriodNode(models.Model):
    ...
    repeat_after = models.CharField(max_length=255, choices=PERIOD_CHOISES, verbose_name=_(u'Повторять через'), default='DAY', blank=True)
...
class PrepaidTraffic(models.Model):
    ...
    """
    traffic_transmit_service = models.ForeignKey(to="TrafficTransmitService", verbose_name=_(u"Услуга доступа по трафику"), related_name="prepaid_traffic")
...
class TrafficTransmitNodes(models.Model):
    traffic_transmit_service = models.ForeignKey(to=TrafficTransmitService, verbose_name=_(u"Услуга доступа по трафику"), related_name="traffic_transmit_nodes")
    #traffic_class     = models.ManyToManyField(to=TrafficClass, verbose_name=_(u'Классы трафика')
    timeperiod        = models.ForeignKey(to=TimePeriod, verbose_name=_(u'Промежуток времени'), null=True, on_delete = models.SET_NULL)
...
class TrafficLimit(models.Model):
    tarif             = models.ForeignKey('Tariff')
    name              = models.CharField(max_length=255, verbose_name=_(u'Название'))
    settlement_period = models.ForeignKey(to=SettlementPeriod, verbose_name=_(u'Период'), blank=True, null=True, on_delete = models.SET_NULL, help_text=_(u"Если период не указан-берётся период тарифного плана. Если установлен автостарт-началом периода будет считаться день привязки тарифного плана пользователю. Если не установлен-старт берётся из расчётного периода"))
    ...
    mode              = models.BooleanField(default=False, blank=True, verbose_name=_(u'За длинну расчётного периода'), help_text=_(u'Если флаг установлен-то количество трафика считается за последние N секунд, указанные в расчётном периоде'))
    action            = models.IntegerField(verbose_name=_(u"Действие"), blank=True, default=0, choices=((0, _(u"Заблокировать пользователя")), (1, _(u"Изменить скорость"))))
    speedlimit = models.ForeignKey("SpeedLimit", verbose_name=_(u"Изменить скорость"), blank=True, null=True, on_delete=models.SET_NULL)
...
class Account(DynamicModel):
    ...
    allow_webcab = models.BooleanField(verbose_name=_(u"Разрешить пользоваться веб-кабинетом"), blank=True, default=True)
    allow_expresscards = models.BooleanField(verbose_name=_(u"Разрешить активацию карт экспресс-оплаты"), blank=True, default=True)
    ...
    systemuser = models.ForeignKey('SystemUser',verbose_name=_(u'Менеджер'), blank=True,null=True, on_delete = models.SET_NULL)
...
class TransactionType(models.Model):
    ...
    allowed_systemusers = models.ManyToManyField("SystemUser", verbose_name = u'Разрешено выполнять', blank=True, null=True)
...
class AccountIPNSpeed(models.Model):
    ...
    static  = models.BooleanField(verbose_name=_(u"Статическая скорость"), help_text=_(u"Пока опция установлена, биллинг не будет менять для этого клиента скорость"), blank=True, default=False)
...
class Card(models.Model):
    ...
    type = models.IntegerField(verbose_name=_(u"Тип"), choices = ( (0, _(u"Экспресс-оплаты"), ), (1, _(u'Хотспот')), (2, _(u'VPN доступ')), (3, _(u'Телефония')),))
    ...
    salecard = models.ForeignKey("SaleCard", verbose_name=_(u"Продана"), blank=True, null=True, on_delete=models.SET_NULL)
...
class Group(models.Model):
    ...
    direction = models.IntegerField(verbose_name=_(u'Направление'), choices=((0, _(u"Входящий")), (1, _(u"Исходящий")), (2, _(u"Вх.+Исх.")),))
    # 1 -sum, 2-max
    type = models.IntegerField(verbose_name=_(u'Тип'), choices=((1, _(u"Сумма классов")), (2, _(u"Максимальный класс"))))
...
class SpeedLimit(models.Model):
    change_speed_type = models.CharField(verbose_name=_(u"Способ изменения скорости"), max_length=32, choices=(("add", "Добавить к текущей"), ("abs","Абсолютное значение",),), blank=True, null=True)    
    speed_units = models.CharField(verbose_name=_(u"Единицы"), max_length = 32,choices=(("Kbps","Kbps",), ("Mbps","Mbps",),("%", "%",)), blank=True, null=True)
...
class TPChangeRule(models.Model):
    ...
    settlement_period = models.ForeignKey(SettlementPeriod, verbose_name=_(u'Расчётный период'), blank=True, null=True, on_delete=models.SET_NULL)
...
class RadiusAttrs(models.Model):
    ...
    account_status = models.IntegerField(choices=((0, _(u'Всегда')), (1, _(u'Активен')), (2, _(u'Не активен')),), default=0, verbose_name=_(u'Статус аккаунта'), help_text=_(u'Добавлять атрибут в Access Accept, если срабатывает условие'))
...
class AddonService(models.Model):    
    ...
    cancel_subscription = models.BooleanField(verbose_name=_(u"Разрешить отключение"), help_text=_(u"Разрешить самостоятельное отключение услуги"), default = True)    
    wyte_period = models.ForeignKey(SettlementPeriod, verbose_name=_(u"Штрафуемый период"), help_text=_(u"Списывать сумму штрафа при досрочном отключении услуги пользователем"),  related_name="addonservice_wyteperiod", blank=True, null=True, on_delete=models.SET_NULL)    
    wyte_cost = models.DecimalField(verbose_name=_(u"Сумма штрафа"), decimal_places=2, max_digits=10, blank=True, default=0)    
    action = models.BooleanField(verbose_name=_(u"Выполнить действие"), blank=True, default=False)    
    nas = models.ForeignKey(Nas, verbose_name=_(u"Сервер доступа"), help_text=_(u"Сервер доступа, на котором будут производиться действия"),  blank=True, null=True, on_delete=models.SET_NULL)    
    service_activation_action = models.TextField(verbose_name=_(u"Действие для активации услуги"), blank=True, default='')    
    service_deactivation_action = models.TextField(verbose_name=_(u"Действие для отключения услуги"), blank=True, default='')    
    deactivate_service_for_blocked_account = models.BooleanField(verbose_name=_(u"Отключать услугу при бловировке аккаунта"), help_text=_(u"Отключать услугу при достижении нулевого баланса или блокировках"), blank=True, default=False)    
    change_speed = models.BooleanField(verbose_name=_(u"Изменить скорость"), help_text=_(u"Изменить параметры скорости при активации аккаунта"), blank=True, default=False) 
    change_speed_type = models.CharField(verbose_name=_(u"Способ изменения скорости"), max_length=32, choices=(("add", _(u"Добавить к текущей")), ("abs",_(u"Абсолютное значение"))), blank=True, null=True)    
    speed_units = models.CharField(verbose_name=_(u"Единицы скорости"), max_length = 32,choices=(("Kbps","Kbps",), ("Mbps","Mbps",),("%", "%",)), blank=True, null=True)    
...
class AddonServiceTarif(models.Model):    
    ...
    activation_count_period = models.ForeignKey(SettlementPeriod, verbose_name=_(u"Расчётный период"), blank=True, null=True)    
    type=models.IntegerField(verbose_name=_(u"Тип активации"), choices=((0, _(u"На аккаунт")), (1, _(u"На субаккаунт")), ),  default=0)# 0-Account, 1-Subaccount
...
class News(models.Model):
    ...
    age = models.DateTimeField(verbose_name=_(u'Актуальна до'), help_text = _(u'Не указывайте ничего, если новость должна отображаться всегда'), blank=True, null=True)
    public = models.BooleanField(verbose_name=_(u'Публичная'), help_text=_(u'Отображать в публичной части веб-кабинета'), default=False)
    private = models.BooleanField(verbose_name=_(u'Приватная'), help_text=_(u'Отображать в приватной части веб-кабинета'), default=False)
...
class RadiusTrafficNode(models.Model):
    ...
    value = models.BigIntegerField(verbose_name=_(u"Объём"), help_text=_(u"Объём, с которого действует указаная цена"), default=0)
    timeperiod = models.ForeignKey(TimePeriod, verbose_name=_(u"Период тарификации"))
    cost = models.DecimalField(verbose_name=_(u"Цена"), help_text=_(u"Цена за единицу тарификации"), default=0, max_digits=30, decimal_places=3)
...
class Switch(models.Model):
    manufacturer = models.ForeignKey(to=Manufacturer, verbose_name=_(u"Производитель"), max_length=250, blank=True, null=True, default='', on_delete=models.SET_NULL)
    model = models.ForeignKey(to=Model, verbose_name=_(u"Модель"), max_length=250, blank=True, null=True, default='', on_delete=models.SET_NULL)
    ...
    city = models.ForeignKey(to=City, verbose_name=_(u"Город"),  blank=True, null=True, default='', on_delete=models.SET_NULL)
    ...
    snmp_version = models.CharField(max_length=10, choices=(('1', "v1",),('2c', "v2c",)), verbose_name=_(u"Версия SNMP"), blank=True, default='v1')#version
    ...
    management_method = models.IntegerField(verbose_name=_(u"Метод SNMP управления"), choices=((0, _(u"Не управлять")),(1, _(u"SSH")),(2, _(u"SNMP")),(3, _(u"Telnet")),(4, _(u"localhost"))),  blank=True, default=1)
    option82 = models.BooleanField(verbose_name=_(u"Опция 82"), default=False)
    option82_auth_type = models.IntegerField(verbose_name=_(u"Тип авторизации по Option82"), choices=((0, _(u"Порт"),), (1, _(u"Порт+MAC"),), (2, _(u"MAC"),)), blank=True, null=True)#1-port, 2 - mac+port, 3-mac
    ...
    option82_template = models.CharField(verbose_name=_(u"Шаблон option82"), choices=(("dlink-32xx",'dlink-32xx'),), blank=True, max_length=256,  default='')
