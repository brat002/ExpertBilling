...
class PeriodicalServiceHistory(models.Model):
    service = models.ForeignKey(
        to=PeriodicalService, blank=True, null=True, on_delete=models.SET_NULL)
    accounttarif = models.ForeignKey(
        to='AccountTarif', on_delete=models.CASCADE)
...
class OneTimeServiceHistory(models.Model):
    onetimeservice = models.ForeignKey(
        OneTimeService, null=True, on_delete=models.SET_NULL)
    created = models.DateTimeField(auto_now_add=True)
...
class Account(DynamicModel):
    ...
    def get_account_tariff(self):
        tariff = Tariff.objects.extra(where=['id=get_tarif(%s)'],
                                      params=[self.id])[:1]
        if tariff:
            return tariff[0]
    ...
    def get_account_tariff_info(self):
        tariff_info = Tariff.objects.extra(where=['id=get_tarif(%s)'],
                                           params=[self.id])[:1]
    ...
    def tariff(self):
        try:
            name = Tariff.objects.extra(where=['id=get_tarif(%s)'],
                                        params=[self.id])[0].name
        except:
...
class Transaction(models.Model):
    ...
    account = models.ForeignKey(
        Account, on_delete=models.CASCADE, verbose_name=_(u"Аккаунт"))
    accounttarif = models.ForeignKey(
        'AccountTarif', blank=True, null=True, on_delete=models.CASCADE)
    type = models.ForeignKey(
        to=TransactionType,
        null=True,
        to_field='internal_name',
    ...
    tarif = models.ForeignKey(
        Tariff, blank=True, null=True, on_delete=models.SET_NULL)
...
class Card(models.Model):
    ...
    activated_by = models.ForeignKey(
        Account, blank=True, null=True, verbose_name=_(u"Аккаунт"))
    ...
    tarif = models.ForeignKey(
        Tariff, blank=True, null=True, verbose_name=_(u"Тариф"))
...
class Dealer(models.Model):
    ...
    bank = models.ForeignKey(
        BankData, blank=True, null=True, on_delete=models.SET_NULL)
...
class TrafficTransaction(models.Model):
    traffictransmitservice = models.ForeignKey(
        TrafficTransmitService, null=True, on_delete=models.SET_NULL)
    account = models.ForeignKey(Account, on_delete=models.CASCADE)
...
class TimeTransaction(models.Model):
    timeaccessservice = models.ForeignKey(
        TimeAccessService, null=True, on_delete=models.SET_NULL)
    account = models.ForeignKey(Account, on_delete=models.CASCADE)
    summ = models.FloatField()
    prev_balance = models.DecimalField(
        verbose_name=(u'Предыдущий баланс'),
...
class TPChangeRule(models.Model):
    ...
    to_tariff = models.ForeignKey(
        Tariff, verbose_name=_(u'На тарифный план'), related_name="to_tariff")
    disabled = models.BooleanField(
...
class SubAccount(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.vpn_ipinuse:
            ...
            if str(self.vpn_ip_address) not in \
                ...
                if self.ipv4_vpn_pool:
                    ...
                    if str(self.vpn_ipinuse.ip) != str(self.vpn_ip_address):
                        ...
                        obj = IPInUse(pool=self.ipv4_vpn_pool,
            ...
            ip = IPInUse(pool=self.ipv4_vpn_pool,
                         ip=self.vpn_ip_address,
                         datetime=datetime.datetime.now())
            ip.save()
            self.vpn_ipinuse = ip
        ...
        if self.vpn_ipv6_ipinuse:
            ...
            ip = IPInUse(pool=self.ipv6_vpn_pool,
                         ip=self.vpn_ipv6_ip_address,
                         datetime=datetime.datetime.now())
            ip.save()
        ...
        if self.ipn_ipinuse:
            ...
            if str(self.ipn_ip_address) not in ['0.0.0.0', '0.0.0.0/32', '', None]:
                ...
                if self.ipv4_ipn_pool:
                    ...
                    if str(self.ipn_ipinuse.ip) != str(self.ipn_ip_address):
                        ...
                        obj = IPInUse(pool=self.ipv4_ipn_pool,
            ...
            ip = IPInUse(pool=self.ipv4_ipn_pool,
                         ip=self.ipn_ip_address,
                         datetime=datetime.datetime.now())
            ip.save()
            self.ipn_ipinuse = ip
...
class SwitchPort(models.Model):
    ...
    protection = models.ForeignKey(
        Hardware, blank=True, null=True, on_delete=models.SET_NULL)
