...
from chartit.validation import clean_sortf_mapf_mts, clean_dps, clean_pdps
...
class DataPool(object):
    ...
    def _group_terms_by_query(self, sort_by_term=None, *addl_grp_terms):
        ...
        sort_grp_fn = lambda (tk, td): tuple(
            chain(str(td['source'].query), [td[t] for t in addl_grp_terms]))
...
class PivotDataPool(DataPool):
    ...
    def __init__(self, series, top_n_term=None, top_n=None, pareto_term=None,
        ...
        self.top_n_term = (top_n_term if top_n_term in self.series.keys()
                           else None)
        self.top_n = (top_n if (self.top_n_term is not None
                                and isinstance(top_n, int)) else 0)
        self.pareto_term = (pareto_term if pareto_term in self.series.keys()
                            else None)
