...
class JQueryAutoCompleteFilter(forms.TextInput):
    ...
    def render_js(self, field_id):
        ...
        return (u"""jQuery('#%s').autocomplete(%s%s);""" %
...
<a href="#" class="selector-search" onclick="user_filter()">\
<img src="%(media_url)shelpdesk_img/selector-search.gif"></a>
<script type="text/javascript">%(js)s</script>''' % template_params)
...
class SelectDateTimeWidget(Widget):
    ...
    def __init__(self, attrs=None, years=None, show_time=True, default=None,
                 null=False):
    ...
    def render(self, name, value, attrs=None):
        ...
        select_html = Select(choices=month_choices).render(
            self.month_field % name, month_val)
        output.append(select_html)

        day_choices = [(i, i) for i in range(1, 32)]
        select_html = Select(choices=day_choices,
                             attrs={'style': 'width: 50px'})
        select_html = select_html.render(self.day_field % name, day_val)
        output.append(select_html)

        year_choices = [(i, i) for i in self.years]
        select_html = Select(choices=year_choices,
                             attrs={'style': 'width: 70px'})
        select_html = select_html.render(self.year_field % name, year_val)
        output.append(select_html)

        if self.show_time:
            html = Input(attrs={'size': 5}).render(
                self.hour_field % name, hour_val)
            output.append('&nbsp;&nbsp;' + html)

            html = Input(attrs={'size': 5}).render(
                self.minute_field % name, minute_val)
            output.append(html)

        null_field = None
        if self.null:
            null_field = CheckboxInput(attrs={'id': self.null_field % name})
            null_field = null_field.render(self.null_field % name,
                                           self.is_null)
    ...
    def value_from_datadict(self, data, files, name):
        ...
        y, m, d, h, mi = (data.get(self.year_field % name, None),
                          data.get(self.month_field % name, None),
                          data.get(self.day_field % name, None),
                          data.get(self.hour_field % name, None) or u'00',
                          data.get(self.minute_field % name, None) or u'00')
        if y and m and d:
