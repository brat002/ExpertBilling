...
words = {
         ...
         u'comment':(u'комментарий, комментария, комментариев', u'Комментариев нет'),
         u'user':(u'пользователь, пользователя, пользователей', u'пользователей нет'),
...
def get_plural(value, arg):
    ...
    if not variants:
        return ''
    return pytils.numeral.get_plural(int(value), variants[0], absence=variants[1])
