...
class RadiusAttrs(models.Model):
    ...
    value = models.CharField(max_length = 255)
        class x8021(models.Model):    account = models.ForeignKey(Account, blank = True)    nas = models.ForeignKey(Nas)    port = models.SmallIntegerField()    typeauth = models.CharField(verbose_name=u"Способ авторизации", choices=AUTH_TYPES, max_length=32)    vlan_accept = models.IntegerField()    vlan_reject = models.IntegerField()    simpleauth = models.BooleanField()    class AddonService(models.Model):    name = models.CharField(max_length=255)    allow_activation = models.BooleanField(default = False)    service_type = models.CharField(max_length=32, choices=((u"Разовая услуга","onetime"),(u"Периодическая услуга","periodical",),))    sp_type = models.CharField(max_length=32, choices=((u"В начале расчётного периода","AT_START"),(u"В конце расчётного периода","AT_END"),(u"На протяжении расчётного периода","GRADUAL"),))    sp_period = models.ForeignKey(SettlementPeriod, related_name="addonservice_spperiod")    timeperiod = models.ForeignKey(TimePeriod)    cost = models.DecimalField(decimal_places=10, max_digits=60)    cancel_subscription = models.BooleanField(default = True)    wyte_period = models.ForeignKey(SettlementPeriod, related_name="addonservice_wyteperiod")    wyte_cost = models.DecimalField(decimal_places=10, max_digits=60)    action = models.BooleanField()    nas = models.ForeignKey(Nas)    service_activation_action = models.CharField(max_length = 8000)    service_deactivation_action = models.CharField(max_length = 8000)    deactivate_service_for_blocked_account = models.BooleanField()    change_speed = models.BooleanField()    change_sped_type = models.CharField(max_length=32, choices=(("Add","add",), ("abs","abs",),))    speed_units = models.CharField(max_length = 32,choices=(("Kbps","Kbps",), ("Mbps","Mbps",),("%", "%",)))    max_tx = models.IntegerField()    max_rx = models.IntegerField()    burst_tx = models.IntegerField()    burst_rx = models.IntegerField()    burst_treshold_tx = models.IntegerField()    burst_treshold_rx = models.IntegerField()    burst_time_tx = models.IntegerField()    burst_time_rx = models.IntegerField()    min_tx = models.IntegerField()    min_rx = models.IntegerField()    priority = models.IntegerField()    class AddonServiceTarif(models.Model):    tarif = models.ForeignKey(Tariff)    service = models.ForeignKey(AddonService)    activation_acount = models.IntegerField()    activation_acount_period = models.ForeignKey(SettlementPeriod)    class AccountAddonService(models.Model):    service = models.ForeignKey(AddonService)    account = models.ForeignKey(Account)    activated = models.DateTimeField()    deactivated = models.DateTimeField()    action_status = models.BooleanField()    speed_status = models.BooleanField()        