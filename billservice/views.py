...
def get_promise(request):
    ...
    if request.method == 'POST':
        ...
        last_promises = Transaction.objects.filter(account=user, type__in=TransactionType.objects.filter(internal_name__in=['PROMISE_PAYMENT', 'PROMISE_DEBIT'])).order_by('-created')[0:10]
