...
class _html2text(sgmllib.SGMLParser):
         ...
         for a in self.a:
             ...
             if a.has_key('href') and a['href'] == attrs['href']:
                 if a.has_key('title') or attrs.has_key('title'):
                         if (a.has_key('title') and attrs.has_key('title') and a['title'] == attrs['title']):
                             match = True
                 else:
                     match = True
    ...
    def handle_tag(self, tag, attrs, start):
        ...
        if tag == "pre" or tag == 'textarea':
            if start:
#                self.o('::')
                self.o('::\n\n') #XXX: added 2006 08 25 by stevek (pre blocks must have at least one blank line before the pre content starts)
                self.startpre = 1
                self.pre = 1
            else:
                self.pre = 0
                self.o("\n") #XXX: added 2006 08 25 by stevek (pre blocks need at least one blank line before unindented content starts)
            #~ self.p()
    ...
    def o(self, data, puredata=0, force=0):
        if not self.quiet: 
            ...
            if self.pre:
                bq += "    "
#                data = data.replace("\n", "\n"+bq)
                data = "  "+data.replace("\n", "\n"+bq) #XXX: added 2006 08 25 by stevek (all pre lines need to be indented)
