...
def notificationssettings_edit(request):
    ...
    if request.method == 'POST': 
        ...
        if id:
            ...
            if  not (request.user.account.has_perm('billservice.change_notificationssettings')):
                messages.error(request, _(u'У вас нет прав на редактирование нотификаций'), extra_tags='alert-danger')
                return HttpResponseRedirect(request.path)
        else:
            
            if  not (request.user.account.has_perm('billservice.add_notificationssettings')):
                messages.error(request, _(u'У вас нет прав на создание нотификаций'), extra_tags='alert-danger')
                return HttpResponseRedirect(request.path)
...
def notificationssettings_delete(request):
    if  not (request.user.account.has_perm('billservice.delete_notificationssettings')):
    ...
    if id:
        try:
            ...
            return {"status": False, "message": _(u"Указанный способ уведомлений не найден %s") % str(e)}
        log('DELETE', request.user, item)
        item.delete()
        return {"status": True}
    else:
        return {"status": False, "message": "Notificationssettings not found"} 
