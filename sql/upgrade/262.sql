...
WHERE free_ip.ipaddress NOT IN 
   (SELECT ip FROM billservice_ipinuse WHERE pool_id=ag_pool_id_ and ((dynamic=False and disabled is null) or (disabled is NULL and ack=False and datetime>=now()-(acct_interval*1 || ' seconds')::INTERVAL) or (last_update is not null and last_update>now()-(acct_interval*1 || ' seconds')::INTERVAL))) ORDER BY RANDOM() LIMIT 1);
