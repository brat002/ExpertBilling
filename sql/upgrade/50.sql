...
BEGIN
    ...
    -- Создаём пользователя
      ...
      ELSIF card_data_.access_type='HotSpotIp+Mac' THEN
         ...
         account_id, username, "password",
         ipn_added, ipn_enabled, need_resync,
         speed, allow_addonservice, ipn_ip_address, ipn_mac_address, pv4_vpn_pool_id)
          VALUES (account_id_, '', '',
    ...
    tariff.active, subaccount.ipv4_vpn_pool_id,tariff.vpn_ippool_id,subaccount.vpn_ip_address,subaccount.ipn_ip_address,subaccount.ipn_mac_address,(SELECT access_type FROM billservice_accessparameters WHERE id=(SELECT access_parameters_id FROM billservice_tariff WHERE id=bsat.tarif_id)) as access_type INTO account_data_
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION card_activate_fn(character varying, character varying, inet,text) OWNER TO postgres;
