...
def generate_graph(image_path, subaccount, interval=None, from_date=None, to_date=None):
    ...
    if interval[1]:    
        ...
        rrd=unicode(u"""%s graph %s -a PNG -h 125 -w 500 --font TITLE:8:Times -t "График загрузки канала субаккаунтом %s за %s (байты)" --lazy -l 0 -v Байты/сек -s %s -e %s DEF:in=%s.rrd:in:AVERAGE  DEF:out=%s.rrd:out:AVERAGE  CDEF:out_neg=out,1,/  CDEF:in_calc=in,1,/ LINE2:in_calc#4169E1 AREA:in_calc#4169E1AA:Incomming  GPRINT:in_calc:MAX:"Max in\\:%%5.1lf%%s" GPRINT:in_calc:AVERAGE:"Avg\\: %%5.1lf %%S" GPRINT:in_calc:LAST:" Current\\: %%5.1lf %%S\\n" LINE2:out_neg#FA0502  AREA:out_neg#FA0502AA:Outgoing GPRINT:out_neg:MAX:"Max out\\:%%4.1lf%%s" GPRINT:out_neg:AVERAGE:"Avg\\: %%5.1lf %%S" GPRINT:out_neg:LAST:" Current\\: %%5.1lf %%S\\n" """ % (RRDTOOL_PATH, image_path, subaccount.username, unicode(interval[0]), from_date, to_date, RRDDB_PATH+'bandwidth_%s'%subaccount.id, RRDDB_PATH+'bandwidth_%s' % subaccount.id,)).encode('utf-8') 
    #print rrd
...
def generate_packets_graph(image_path, subaccount, interval, from_date=None, to_date=None):
    ...
    if interval[1]:    
        ...
        from_date, to_date = int(from_date), int(to_date)
        rrdf=unicode(u"""%s graph %s -a PNG -h 125 -w 500 --font TITLE:8:Times -t "График загрузки канала субаккаунтом %s за %s (пакеты)"  --lazy -l 0 -v Packets/s -s %s -e %s DEF:packets_in=%s.rrd:packets_in:AVERAGE  DEF:packets_out=%s.rrd:packets_out:AVERAGE  CDEF:out=packets_out,1,/  CDEF:in=packets_in,1,/ LINE2:in#4169E1 AREA:in#4169E1AA:Incomming   GPRINT:in:MAX:"Max in\\:%%5.1lf%%s" GPRINT:in:AVERAGE:"Avg\\: %%5.1lf %%S" GPRINT:in:LAST:" Current\\: %%5.1lf %%S\\n" LINE2:out#FA0502  AREA:out#FA0502AA:Outgoing GPRINT:out:MAX:"Max out\\:%%5.1lf%%s" GPRINT:out:AVERAGE:"Avg\\: %%5.1lf %%S" GPRINT:out:LAST:" Current\\: %%5.1lf %%S\\n"  """ % (RRDTOOL_PATH, image_path, subaccount.username, interval[0], from_date, to_date, RRDDB_PATH+'bandwidth_%s'%subaccount.id, RRDDB_PATH+'bandwidth_%s' % subaccount.id,)).encode('utf-8')
...
def generate_nas_graph(image_path, nas, interval, from_date=None, to_date=None):
    ...
    if interval[1]:
        ...
        from_date, to_date = int(from_date), int(to_date)    
        rrd=unicode(u"""%s graph %s -a PNG -h 125 -w 500 --font TITLE:8:Times -t "График загрузки канала на %s" --lazy -l 0 -v Байты/сек -s %s -e %s DEF:in=%s.rrd:in:AVERAGE  DEF:out=%s.rrd:out:AVERAGE  CDEF:out_neg=out,1,/  CDEF:in_calc=in,1,/ LINE2:in_calc#4169E1 AREA:in_calc#4169E1AA:Incomming   GPRINT:in_calc:MAX:"Max in\\:%%5.1lf%%s" GPRINT:in_calc:AVERAGE:"Avg\\: %%5.1lf %%S" GPRINT:in_calc:LAST:" Current\\: %%5.1lf %%S\\n" LINE2:out_neg#FA0502  AREA:out_neg#FA0502AA:Outgoing GPRINT:out_neg:MAX:"Max out\\:%%4.1lf%%s" GPRINT:out_neg:AVERAGE:"Avg\\: %%5.1lf %%S" GPRINT:out_neg:LAST:" Current\\: %%5.1lf %%S\\n" """ % (RRDTOOL_PATH, image_path, nas.name, from_date, to_date, RRDDB_PATH+'bandwidth_nas_%s'%nas.id, RRDDB_PATH+'bandwidth_nas_%s' % nas.id,)).encode('utf-8')
    #print rrd
    status, output = commands.getstatusoutput(rrd)
    return status, rrd, output
...
def generate_nas_packets_graph(image_path, nas, interval, from_date=None, to_date=None):
    ...
    if interval[1]:
        ...
        from_date, to_date = int(from_date), int(to_date)
        rrd=unicode(u"""%s graph %s -a PNG -h 125 -w 500 --font TITLE:8:Times -t "График загрузки канала на %s (пакеты)" --lazy -l 0 -v Пакеты/сек -s %s -e %s DEF:packets_in=%s.rrd:packets_in:AVERAGE  DEF:packets_out=%s.rrd:packets_out:AVERAGE  CDEF:out=packets_out,1,/  CDEF:in=packets_in,1,/ LINE2:in#4169E1 AREA:in#4169E1AA:Incomming   GPRINT:in:MAX:"Max in\\:%%5.1lf%%s" GPRINT:in:AVERAGE:"Avg\\: %%5.1lf %%S" GPRINT:in:LAST:" Current\\: %%5.1lf %%S\\n" LINE2:out#FA0502  AREA:out#FA0502AA:Outgoing GPRINT:out:MAX:"Max out\\:%%5.1lf%%s" GPRINT:out:AVERAGE:"Avg\\: %%5.1lf %%S" GPRINT:out:LAST:" Current\\: %%5.1lf %%S\\n"  """ % (RRDTOOL_PATH, image_path, nas.name, from_date, to_date, RRDDB_PATH+'bandwidth_nas_%s'%nas.id, RRDDB_PATH+'bandwidth_nas_%s' % nas.id,)).encode('utf-8')
    #print rrd
    status, output = commands.getstatusoutput(rrd)
    return status, rrd, output
...
def generate_nas_sessions_graph(image_path, nas, interval, from_date=None, to_date=None):
    ...
    if interval[1]:
        ...
        from_date, to_date = int(from_date), int(to_date)
        rrd=unicode(u"""%s graph %s -a PNG -h 125 -w 500 --font TITLE:8:Times -t "График количества VPN сессий на %s " --lazy -l 0 -v Сессий -s %s -e %s DEF:sessions=%s.rrd:sessions:AVERAGE LINE2:sessions#4169E1 AREA:sessions#4169E1AA:Sessions   GPRINT:sessions:MAX:"Max in\\:%%5.1lf%%s" GPRINT:sessions:AVERAGE:"Avg\\: %%5.1lf %%S" GPRINT:sessions:LAST:" Current\\: %%5.1lf %%S\\n" """ % (RRDTOOL_PATH, image_path, nas.name, from_date, to_date,  RRDDB_PATH+'bandwidth_nas_sessions_%s' % nas.id,)).encode('utf-8')
...
def subaccounts_period_stat(request):
    ...
    period = "%s-%s" % (datetime.datetime.fromtimestamp(a).strftime('%d.%m.%y %H:%M:%S'),datetime.datetime.fromtimestamp(b).strftime('%d.%m.%y %H:%M:%S'),)

#    print period
    #to_date = datetime.datetime.fromtimestamp(b)    
...
#            print a,b,c
    ...
    return {'nas':nas,"filenames":filenames}
...
#    print period
    ...
    for item in items:
        ...
        generate_nas_graph(IMAGE_PATH+'nasses/'+bytes_filename,  item, [period,''],a,b)
        generate_nas_packets_graph(IMAGE_PATH+'nasses/'+packets_filename, item, [period,''],a,b )
        generate_nas_sessions_graph(IMAGE_PATH+'nasses/'+sessions_filename, item,  [period,''],a,b)
                                    
        #x,n,c=generate_graph(IMAGE_PATH+'subaccounts/'+bytes_filename,  subaccount, [period,''], a, b)
        #x,n,c=generate_packets_graph(IMAGE_PATH+'subaccounts/'+packets_filename, subaccount, [period, ''], a, b)
        filenames.append((None, bytes_filename, packets_filename, sessions_filename))
        #print x,n,c
    return {'nas':item,"filenames":filenames}
