...
def make_dict(lst, fields_list, datetime_fmt, encoding='utf-8', time_fmt='', ):
    ...
    for field in fields_list:
        if field=='DATETIME':
            ...
            result_dict[field]=lst[i].decode(encoding, 'utf-8')
        i+=1
...
if __name__=='__main__':
    ...
    if config.has_section(payment_system):
        ...
        for file in os.listdir(curdir+folder_in):
            buffer=[]
            if not (fnmatch.fnmatch(file, file_mask) and not fnmatch.fnmatch(file, exclude_mask) and not file=='.svn'): continue
