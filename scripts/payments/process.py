...
INSERT INTO billservice_transaction(
            ...
            created)
    VALUES (%(ACC_ID)s, '%(PAYMENT_ID)s', '%(COMMENT)s', '%(PAYMENT_TYPE)s', True, %(SUM)s,  '%(DATETIME)s');
"""
reversal_transaction_pattern=u"""
INSERT INTO billservice_transaction(
            account_id, bill, description, type_id, approved, (-1)*summ,
            created)
    VALUES (%(ACC_ID)s, '%(PAYMENT_ID)s', '%(COMMENT)s', '%(PAYMENT_TYPE)s', True, (-1)*%(SUM)s,  '%(DATETIME)s');
"""
...
def check_dublicates(cur, param_dict):
    ...
    WHERE account_id=%(ACC_ID)s and bill='%(PAYMENT_ID)s'  and type_id='%(PAYMENT_TYPE)s' and approved=True and summ=(-1)*%(SUM)s and
            created='%(DATETIME)s'
            """ % param_dict)
