...
class AccountData(tuple):
    'AccountData(account_id, username, ipn_mac_address, time_access_service_id, password, nas_id, vpn_ip_address, tarif_id, access_type, account_status, balance_blocked, ballance, disabled_by_limit, vpn_speed, tarif_active, allow_vpn_null, allow_vpn_block, ipn_ip_address, ipn_speed, assign_dhcp_null, assign_dhcp_block)' 
    ...
    def __new__(cls, account_id, username, ipn_mac_address, time_access_service_id, password, nas_id, vpn_ip_address, tarif_id, access_type, account_status, balance_blocked, ballance, disabled_by_limit, vpn_speed, tarif_active, allow_vpn_null, allow_vpn_block, ipn_ip_address, ipn_speed, assign_dhcp_null, assign_dhcp_block, associate_pptp_ipn_ip, associate_pppoe_mac):
    ...
    def _make(cls, iterable, new=tuple.__new__, len=len):
        ...
        if len(result) != 23:
            raise TypeError('Expected 23 arguments, got %d' % len(result))
        return result 
    ...
    def _asdict(t):
        ...
        return {'account_id': t[0], 'username': t[1], 'ipn_mac_address': t[2], 'time_access_service_id': t[3], 'password': t[4], 'nas_id': t[5], 'vpn_ip_address': t[6], 'tarif_id': t[7], 'access_type': t[8], 'account_status': t[9], 'balance_blocked': t[10], 'ballance': t[11], 'disabled_by_limit': t[12], 'vpn_speed': t[13], 'tarif_active': t[14], 'allow_vpn_null': t[15], 'allow_vpn_block': t[16], 'ipn_ip_address': t[17], 'ipn_speed': t[18], 'assign_dhcp_null': t[19], 'assign_dhcp_block': t[20], 'associate_pptp_ipn_ip': t[21], 'associate_pppoe_mac':t[22]} 

    def _replace(self, **kwds):
        'Return a new AccountData object replacing specified fields with new values'
        result = self._make(map(kwds.pop, ('account_id', 'username', 'ipn_mac_address', 'time_access_service_id', 'password', 'nas_id', 'vpn_ip_address', 'tarif_id', 'access_type', 'account_status', 'balance_blocked', 'ballance', 'disabled_by_limit', 'vpn_speed', 'tarif_active', 'allow_vpn_null', 'allow_vpn_block', 'ipn_ip_address', 'ipn_speed', 'assign_dhcp_null', 'assign_dhcp_block', 'associate_pptp_ipn_ip', 'associate_pppoe_mac'), self))
        if kwds:
