...
nf_sql = {'nas':"SELECT ipaddress, id from nas_nas;",
          'accounts':"SELECT ba.id as account_id,ba.vpn_ip_address,ba.ipn_ip_address, bacct.id as acctf_id, bacct.tarif_id, ba.nas_id FROM billservice_account AS ba JOIN billservice_accounttarif AS bacct ON bacct.id=(SELECT id FROM billservice_accounttarif AS att WHERE att.account_id=ba.id and att.datetime<%s ORDER BY datetime DESC LIMIT 1);",
...
rad_sql = \
        {'accounts'  :"""SELECT ba.id, ba.username, ba.ipn_mac_address, bt.time_access_service_id, 
         ...
         'nas'      :"""SELECT id, secret, type, multilink, ipaddress, identify, speed_vendor_1, speed_vendor_2, speed_attr_id1, speed_attr_id2, speed_value1, speed_value2 FROM nas_nas ORDER BY id, ipaddress, identify;""",
