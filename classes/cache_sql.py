...
nf_sql = {'nas':"SELECT id, ipaddress from nas_nas;",
          'accounts':"SELECT ba.id as account_id, array(SELECT COALESCE(sa.vpn_ip_address, '0.0.0.0'::inet) || '|' || COALESCE(sa.ipn_ip_address, '0.0.0.0'::inet) || '|' || COALESCE(sa.nas_id, 0) FROM billservice_subaccount as sa WHERE sa.account_id=ba.id) as addresses, bacct.id as acctf_id, bacct.tarif_id FROM billservice_account AS ba JOIN billservice_accounttarif AS bacct ON bacct.id=(SELECT id FROM billservice_accounttarif AS att WHERE  att.account_id=ba.id and date_trunc('second', att.datetime)<%s ORDER BY datetime DESC LIMIT 1) WHERE  deleted is Null ;",
