...
class RPCServer(object):
    ...
    def get_accounts_for_tarif(self, tarif_id, cur=None, connection=None, add_data = {}):
        ...
        if tarif_id==-3000:
            ...
            FROM billservice_account AS acc 
            LEFT JOIN billservice_organization as org ON org.account_id=acc.id 
            WHERE  %s IN (SELECT id FROM billservice_tariff WHERE systemgroup_id is Null or systemgroup_id IN (SELECT systemgroup_id FROM billservice_systemuser_group WHERE systemuser_id=%s)) and acc.id not IN (SELECT account_id FROM billservice_organization) ORDER BY acc.username ASC;""", (add_data['USER_ID'][1],) )

        elif tarif_id==-5000:#Юр лица
            cur.execute("""SELECT acc.id, acc.username, acc.fullname, acc.email, acc.nas_id, acc.ipn_status, acc.ipn_added, acc.suspended, acc.created, acc.ballance, acc.credit, acc.contract, acc.disabled_by_limit, acc.balance_blocked, acc."comment", acc.status, acc.last_balance_null, (SELECT name FROM nas_nas where id = acc.nas_id) AS nas_name, (SELECT name FROM billservice_tariff WHERE id=get_tarif(acc.id)) as tarif_name, org.id as org_id, org.name as org_name,ARRAY(SELECT DISTINCT vpn_ip_address FROM billservice_subaccount as subacc WHERE subacc.account_id=acc.id) as vpn_ips,ARRAY(SELECT DISTINCT ipn_ip_address FROM billservice_subaccount as subacc WHERE subacc.account_id=acc.id) as ipn_ips,ARRAY(SELECT DISTINCT ipn_mac_address FROM billservice_subaccount as subacc WHERE subacc.account_id=acc.id) as ipn_macs,(SELECT True FROM radius_activesession WHERE account_id=acc.id and session_status='ACTIVE') as account_online
            FROM billservice_account AS acc 
            LEFT JOIN billservice_organization as org ON org.account_id=acc.id 
            WHERE %s IN (SELECT id FROM billservice_tariff WHERE systemgroup_id is Null or systemgroup_id IN (SELECT systemgroup_id FROM billservice_systemuser_group WHERE systemuser_id=%s)) and acc.id IN (SELECT account_id FROM billservice_organization)  ORDER BY acc.username ASC;""", (add_data['USER_ID'][1],) )
